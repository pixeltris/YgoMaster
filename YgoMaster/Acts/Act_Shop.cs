...
namespace YgoMaster
{
    ...
    {
        // TODO: Put this somewhere else (called by Act_ShopPurchase / OnSoloChapterComplete)
        ...
        void Act_ShopGetList(GameServerWebRequest request, int targetShopId = 0)
        {
            ...
            foreach (ShopItemInfo shopItem in Shop.AllShops.Values)
            {
                ...
                long packBuyLimit;
                bool isNew;
                if (request.Player.ShopState.GetAvailability(shopItem, out packBuyLimit, out expireTime, out isNew) == PlayerShopItemAvailability.Hidden)
                {
                    continue;
                }
                int buyLimit = 0;
                ...
                switch (shopItem.Category)
                {
                    case ShopCategory.Pack:
                        ...
                        const string packBuyLimitStr = "{BUYLIMIT}";
                ...
                foreach (ShopItemPrice price in shopItem.Prices)
                {
                    ...
                    if (shopItem.PackType != ShopPackType.None)
                    {
                        ...
                        {
                            if (packBuyLimit > 0 && price.TextArgs[0] > packBuyLimit)
                            {
                            ...
                            pop = "IDS_SHOP_BUY_BUTTON_ADS_CARDPACK_10_SR";
                        ...
                        ShopOddsInfo odds = shopItem.GetOdds(Shop);
                        if ((price.TextArgs.Count > 0 || shopItem.Prices.Count == 1) &&
                            odds != null && shopItem.GetFinalCardRarityGuarantee(odds) == CardRarity.SuperRare)
                        {
                            pop = "IDS_SHOP_BUY_BUTTON_ADS_CARDPACK_10_SR_SET";
                        }
                    }
                ...
                switch (shopItem.PackType)
                {
                    ...
                    case ShopPackType.Bonus:
                        ...
                        break;
                    default:
                        cardList[shopItem.ShopId.ToString()] = shopItem.Cards.ToArray();
                        break;
                }
            }
            if (targetShopId > 0)
        ...
        void Act_ShopPurchase(GameServerWebRequest request)
        {
            ...
            {
                if (request.Player.ShopState.GetAvailability(shopItem) != PlayerShopItemAvailability.Available)
                {
                        ...
                        else
                        {
                            ...
                            switch (shopItem.Category)
                            {
                                case ShopCategory.Pack:
                                    ...
                                    int packCount = 1;
                                    if (price.TextArgs.Count > 0 && price.TextArgs[0] > 0)
                                    {
                                        // NOTE: Anything above 10 packs makes the client bug out
                                        packCount = Math.Min(10, price.TextArgs[0]);
                                    }
                                    const int extraGuranteePackCount = 10;
                                    ...
                                    else
                                    {
                                        ...
                                        for (int packIndex = 0; packIndex < packCount; packIndex++)
                                        {
                                            ...
                                            CardRarity rarityGuarantee = CardRarity.None;
                                            for (int cardIndex = 0; cardIndex < shopItem.CardNum; cardIndex++)
                                            {
                                                ...
                                                if (cardIndex == shopItem.CardNum - 1)
                                                {
                                                    ...
                                                    if ((packIndex + 1) == extraGuranteePackCount)
                                                    {
                                                        rarityGuarantee = isUltraRareGuaranteed ? CardRarity.UltraRare : CardRarity.SuperRare;
                                                    }
                                                }
                                                matches.Clear();
                                                int num = cardIndex + 1;
                                                ...
                                                double rarityPercent = rand.NextDouble() * 100;
                                                ...
                                                int foundCardId = 0;
                                                List<int> shuffledCards = Shuffle(rand, match.Standard ? Shop.StandardPack.Cards : shopItem.Cards);
                                                while (rarity > 0)
                                                {
                                                    foreach (int cardId in shuffledCards)
                                                    {
                                                        int cardRarity;
                                                        if (CardRare.TryGetValue(cardId, out cardRarity) && cardRarity == (int)rarity)
                                                        {
                                                            foundCardId = cardId;
                                                            break;
                                                        }
                                                        ...
                                                        switch (secretPack.SecretType)
                                                        {
                                                            ...
                                                            case ShopItemSecretType.Find:
                                                                if (!foundSecrets.Contains(secretPack.ShopId) &&
                                                                    request.Player.ShopState.GetAvailability(secretPack) == PlayerShopItemAvailability.Available)
                                                                {
                                                                ...
                                                                shopIdsToUpdate.Add(secretPack.ShopId);
                                        ...
                                        if (packInfos.Count > 0)
                                        {
                                            ...
                                            if (!Shop.PackShopImagesByCardId.TryGetValue(shopItem.IconMrk, out packImageName) &&
                                                !Shop.PackShopImagesByCardId.TryGetValue(0, out packImageName))
                                            {
                                                packImageName = "CardPackTex01_0000";
                                            }
                                            gacha["effects"] = new Dictionary<string, object>()
                                            {
                                                { "isPickup", isPickup },
                                                { "imageName", packImageName },
                                                { "isNextFinalizedUR", (packCount == extraGuranteePackCount && !pulledUltraRare) },
                                        ...
                                        if (packCount == extraGuranteePackCount)
                                        {
                                            request.Player.ShopState.SetUltraRareGaurantee(shopItem.Id, !pulledUltraRare);
                                            if (pulledUltraRare)
                                            {
            ...
            if (success)
            {
                if (shopItem != null)
                {
                    ...
                    if (request.Player.ShopState.GetAvailability(shopItem) == PlayerShopItemAvailability.Hidden)
                    {
            ...
            if (!success)
            {
                // TODO: Find the correct error response
                request.ErrorCode = 1;
        ...
        void Act_GachaGetProbability(GameServerWebRequest request)
        {
            ...
            if (Shop.PacksByPackId.TryGetValue(packId, out item) && item.Cards.Count > 0)
            {
                ...
                if (odds != null)
                {
                    ...
                    foreach (int cardId in item.Cards)
                    {
                        int rarity;
                        if (CardRare.TryGetValue(cardId, out rarity) && rarity > 0)
                        {
                            if (!cardRarityCount.ContainsKey((CardRarity)rarity))
                            {
                                cardRarityCount[(CardRarity)rarity] = 1;
                            }
                            else
                            {
                                cardRarityCount[(CardRarity)rarity]++;
                            }
                        }
                    }
