...
/*namespace YgomGame
{
    unsafe static class DeckSelectViewController2
    {
        delegate void Del_SelectDeck(IntPtr thisPtr, int id);
        static Hook<Del_SelectDeck> hookSelectDeck;

        static DeckSelectViewController2()
        {
            IL2Assembly assembly = Assembler.GetAssembly("Assembly-CSharp");
            IL2Class classInfo = assembly.GetClass("DeckSelectViewController2", "YgomGame");
            hookSelectDeck = new Hook<Del_SelectDeck>(SelectDeck, classInfo.GetMethod("SelectDeck"));
        }

        static void SelectDeck(IntPtr thisPtr, int id)
        {
            Console.WriteLine("????set deck " + id);
            hookSelectDeck.Original(thisPtr, id);
        }
    }
}*/
...
/*namespace YgomGame.DeckBrowser
{
    unsafe static class DeckBrowserViewController
    {
        static IL2Method methodCompleteDialogOnSelectDeck;

        delegate void Del_SelectDeck(IntPtr thisPtr, int mode, int deckId, int tournamentId, int rentalId);
        static Hook<Del_SelectDeck> hookSelectDeck;

        delegate void Del_CompleteDialogOnSelectDeck(IntPtr thisPtr, IntPtr handle);
        static Hook<Del_CompleteDialogOnSelectDeck> hookCompleteDialogOnSelectDeck;

        static DeckBrowserViewController()
        {
            IL2Assembly assembly = Assembler.GetAssembly("Assembly-CSharp");
            IL2Class classInfo = assembly.GetClass("DeckBrowserViewController", "YgomGame.DeckBrowser");
            methodCompleteDialogOnSelectDeck = classInfo.GetMethod("CompleteDialogOnSelectDeck");
            hookSelectDeck = new Hook<Del_SelectDeck>(SelectDeck, classInfo.GetMethod("SelectDeck"));
            //hookCompleteDialogOnSelectDeck = new Hook<Del_CompleteDialogOnSelectDeck>(CompleteDialogOnSelectDeck, classInfo.GetMethod("CompleteDialogOnSelectDeck"));
        }
        ...
        static void SelectDeck(IntPtr thisPtr, int mode, int deckId, int tournamentId, int rentalId)
        {
            ...
            if (roomView != IntPtr.Zero)
            {
                Console.WriteLine("!!!!!!!!set deck " + mode + " " + deckId + " " + tournamentId + " " + rentalId);
                methodCompleteDialogOnSelectDeck.Invoke(thisPtr, new IntPtr[] { YgomSystem.Network.Handle.CreateFakeHandle() });
                return;
            }
            else
            {
                Console.WriteLine("NOPE " + mode + " " + deckId + " " + tournamentId + " " + rentalId);
            }
            hookSelectDeck.Original(thisPtr, mode, deckId, tournamentId, rentalId);
        }

        /*static void CompleteDialogOnSelectDeck(IntPtr thisPtr, IntPtr handle)
        {
            Console.WriteLine("COMPLETEEEEEEEE");
            hookCompleteDialogOnSelectDeck.Original(thisPtr, handle);
        }*//*
    }
}*/
...
/*namespace YgomSystem.Network
{
    unsafe static class Handle
    {
        static Dictionary<IntPtr, IntPtr> fakeHandles = new Dictionary<IntPtr, IntPtr>();

        static IL2Class handleClassInfo;
        static IL2Method handleCtor;
        static IL2Class requestStructureClassInfo;
        static IL2Method requestStructureCtor;
        static IL2Method requestStructureComplete;
        static IL2Field requestStructureStatus;

        static IL2Method m;

        delegate IntPtr Del_AddCompleteEvent(IntPtr thisPtr, IntPtr e);
        static Hook<Del_AddCompleteEvent> hookAddCompleteEvent;

        static Handle()
        {
            IL2Assembly assembly = Assembler.GetAssembly("Assembly-CSharp");
            requestStructureClassInfo = assembly.GetClass("NetworkMain").GetNestedType("RequestStructure");
            requestStructureCtor = requestStructureClassInfo.GetMethod(".ctor");
            requestStructureComplete = requestStructureClassInfo.GetMethod("Complete");
            requestStructureStatus = requestStructureClassInfo.GetField("status");
            handleClassInfo = assembly.GetClass("Handle", "YgomSystem.Network");
            handleCtor = requestStructureClassInfo.GetMethod(".ctor");

            IL2Class c = assembly.GetClass("EventHandler", "YgomSystem.Network");
            m = c.GetMethod("Invoke");
            //System.Diagnostics.Debugger.Launch();

            //hookAddCompleteEvent = new Hook<Del_AddCompleteEvent>(AddCompleteEvent, handleClassInfo.GetMethod("AddCompleteEvent"));
        }

        static IntPtr AddCompleteEvent(IntPtr thisPtr, IntPtr e)
        {
            Console.WriteLine("add complete " + thisPtr.ToInt64());
            IntPtr structure;
            hookAddCompleteEvent.Original(thisPtr, e);
            if (fakeHandles.TryGetValue(thisPtr, out structure))
            {
                //Import.Method.il2cpp_runtime_invoke(
                fakeHandles.Remove(thisPtr);
                Console.WriteLine("fake complete " + e.ToInt64());
                int status = 1;// COMPLETE
                requestStructureStatus.SetValue(structure, new IntPtr(&status));
                //requestStructureComplete.Invoke(structure);
                m.Invoke(e, new IntPtr[] { thisPtr });
                return thisPtr;
            }
            else
            {
        ...
        public static IntPtr CreateFakeHandle(string command = "fake")
        {
            IntPtr requestStructureObj = Import.Object.il2cpp_object_new(requestStructureClassInfo.ptr);
            IL2Object param = YgomMiniJSON.Json.Deserialize("{\"test\":1000}");
            requestStructureCtor.Invoke(requestStructureObj, new IntPtr[] { new IL2String(command).ptr, param.ptr, IntPtr.Zero });
            IntPtr handleObj = Import.Object.il2cpp_object_new(handleClassInfo.ptr);
            handleCtor.Invoke(handleObj, new IntPtr[] { requestStructureObj });
            fakeHandles[handleObj] = requestStructureObj;
            int status = 1;// COMPLETE
            requestStructureStatus.SetValue(requestStructureObj, new IntPtr(&status));
            Console.WriteLine(handleObj.ToInt64());
            return handleObj;
        }
    }
    ...
    {
        delegate IntPtr Del_Deck_set_select_deck(int _mode_, int _deck_id_);
        static Hook<Del_Deck_set_select_deck> hookDeck_set_select_deck;

        static API()
        {
            IL2Assembly assembly = Assembler.GetAssembly("Assembly-CSharp");
            IL2Class classInfo = assembly.GetClass("API", "YgomSystem.Network");
            hookDeck_set_select_deck = new Hook<Del_Deck_set_select_deck>(Deck_set_select_deck, classInfo.GetMethod("Deck_set_select_deck"));
        }

        static IntPtr Deck_set_select_deck(int _mode_, int _deck_id_)
        {
            IntPtr manager = YgomSystem.Menu.ContentViewControllerManager.GetManager();
            IntPtr roomView = YgomSystem.UI.ViewControllerManager.GetViewController(manager, YgomGame.Room.RoomCreateViewController.ClassInfo.IL2Typeof());
            //if (roomView != IntPtr.Zero)
            //{
            //    Console.WriteLine("do fake");
            //    return Handle.CreateFakeHandle();
            //}
            return hookDeck_set_select_deck.Original(_mode_, _deck_id_);
        }
    }
}*/
